...
left join persona as vendedor on vendedor.idPersona=usuario.Persona_id
 where ficha_venta.Eliminado=0 $sqlVBO_Estado_Venta_BO  and DATE(ficha_venta.Fecha_Registro_Vendedor)>='".$FechaInicio."' and DATE(ficha_venta.Fecha_Registro_Vendedor)<='".$FechaFin."' order by ficha_venta.Fecha_Registro_Vendedor"); 
...
?>
            ...
            <?php foreach($Fichas_Venta as $item): ?>
                <tr>
                    <td><?php echo $item ['idFicha_Venta'];?></td>                    
                    ...
                    <?php 
                        $RE_Dist_Entrega=str_pad($item['RE_Dist_Entrega_Producto'],6,"0", STR_PAD_LEFT); 
                        $RE_Prov_Entrega=substr($RE_Dist_Entrega,0,4);
                        $RE_Dpto_Entrega=substr($RE_Dist_Entrega,0,2);
                    ?>
                    <td><?php echo $arrayDpto[$RE_Dpto_Entrega]['Descripcion'];?></td>
                    <td><?php echo $arrayProv[$RE_Prov_Entrega]['Descripcion'];?></td>
